...
# ==== Eyebrow ROI tweak ====
EYEBROW_SHRINK_PX    = int(os.getenv("EYEBROW_SHRINK_PX", "0"))
...
def annotate_fail_eyebrows(image, brow_polys, results):
    """
    Vẽ 2 ô fail riêng (left/right) theo BB bao polygon lông mày nếu không pass.
    - Dùng _draw_labeled_box để nhất quán style với các phần khác.
    - Vẫn vẽ viền polygon mảnh để người xem “đối chiếu ROI”.
    """
    out = image.copy()
    ...
    for side, poly, key in [
        ...
        if poly is None or status == "pass":
            continue

        # 1) Viền polygon mảnh (để đối chiếu vùng)
        cv2.polylines(out, [poly.astype(np.int32)], isClosed=True, color=FAIL_BOX_COLOR, thickness=1)
...
def check_leg_skin_exposure(test_image, pose_landmarks, gender="male"):
    ...
    def make_poly(side="left"):
        ...
        # === NEW: dời điểm START xuống khỏi HIP một đoạn hướng về KNEE ===
        v_hk = knee - hip
        ...
        start_pt = hip + v_hk_dir * start_shift   # bắt đầu ở giữa đùi, không phải hip nữa

        # === END point như cũ, tùy giới tính ===
